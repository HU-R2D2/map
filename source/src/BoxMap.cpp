...
namespace r2d2
{
    namespace map
    {
        ...
        std::shared_ptr<BoxInfo**> BoxMap::get_map_area_2d(const Box & area, const Box & pixel_size)
        {
                ...
                if (box.intersects(map[j].first)){
                    ...
                    if (!box.contains(map[j].first)){
                        ...
                        if (temp_box.first.get_bottom_left().get_x() < box.get_bottom_left().get_x()){
                            ...
                            temp_box = std::pair<Box, BoxInfo>{ Box{ Coordinate{ box.get_bottom_left().get_x(), temp_box.first.get_bottom_left().get_y(), temp_box.get_bottom_left().get_z() }, temp_box.first.get_top_right() }, temp_box.second };
                        }
                        if (temp_box.first.get_top_right().get_x() > box.get_bottom_left().get_x()){
                            ...
                            temp_box = std::pair<Box, BoxInfo>{ Box{ temp_box.first.get_bottom_left(), Coordinate{ box.get_top_right().get_x(), temp_box.first.get_top_right().get_y(), temp_box.get_top_right().get_z() } }, temp_box.second };
                        }
                        ...
                        if (temp_box.first.get_bottom_left().get_z() > box.get_bottom_left().get_z()){
                            ...
                            temp_box = std::pair<Box, BoxInfo>{Box{ Coordinate{ temp_box.first.get_bottom_left().get_x(), temp_box.get_bottom_left().get_y(), box.get_top_right().get_z() }, temp_box.first.get_top_right() }, temp_box.second};
                        }
                    }
                }
            }
